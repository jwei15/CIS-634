...
\begin{abstract}
This project is mainly a practice on programming in ML. In this project, we conducted several basic operations, such as function application, pattern matching, etc., in HOL. This report is basically a summary of each of the practice problems. For each of the problems, this report contains:
\begin{itemize}
...
Apart of being a summary on the problems, this report itself is a practice on generating and formatting of \LaTeX{} report files. This report used several packages for formatting the document, including:
\begin{itemize}
...
\item \emph{holtex.sty} and \emph{holtexbasic.sty}, which were provided in professor Shiu-Kai Chin's sample report
\end{itemize}
...
\item assign values to $e1B$, a pair, and $listB$, a list of pairs, using pattern matching
\item assign values to $elC1$,  $elC1$, $elC2$, $elC3$, $elC4$ and $elC5$ as they are specified, using pattern matching
\end{enumerate}

\section{Relevant Code}
\label{sec:3.4.1-relevant-code}

According to the requirements of this problem, we will need to define \emph{listA}, then we will attempt to "extract" some values out of \emph{listA}, using pattern matching. Please refer to the code below:
...
The following case also failed, this is because $1$ is a data of type \emph{int} while $[x3]$ is a list of \emph{string}, and a \emph{int} type value can not be insert to the front of a \emph{string} list. Therefore the calculation failed.  
\begin{session}
\begin{scriptsize}
\begin{verbatim}
